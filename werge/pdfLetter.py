...
class PdfLetter:
    ...
    """
        PdfLetter is responsible for the merge between the json content and the field_contents passed.
    ...
    def __init__(self,style_sheet,page_style,default_style,doc,title:str,field_contents:dict,*args,**kwargs):
    ...
    def _add_styles (self,new_style:json)->None:
    ...
    def add_table_reference (self,jsf_tables:json)->Table:
        ...
        for row in jsf_tables["rows"][1:]:
            t_data.append([r["content"].format(**self.field_contents[r['paragraph_key']]) for r in list(row.values())[0]] )
    ...
    @classmethod
    def from_json_file (cls, format_file:json,field_contents:dict,file_name:str):
        """parses the Json file to create an instance of PdfFile, calling the appropriate methods to construct the story and set styles.
            ...
            args:
                ...
                field_contents(ordered dict): An Ordered dictionary that contains keys called by a content item's "paragraph_key" key.
                each value in the field_contents dictionary should be a list or tuple, even if it contains just one item.
        ...
        pdfFile._add_styles(format_file['text_styles'])
